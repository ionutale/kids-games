...
async function solveDeck(page, max = 25) {
  for (let i = 0; i < max; i++) {
...
test.describe('Speed Match E2E', () => {
  ...
  test('Next Level loads a fresh deck for the new level (no stale board)', async ({ page }) => {
    test.setTimeout(90000);
    await page.goto('/games/speed-match/play/1?seed=13');
    await solveDeck(page);
    await expect(page.locator('.win-overlay')).toBeVisible();
    ...
    // level 2 deck size is 10: a fresh deck starts at 1/10
    await expect(page.locator('.hud-item').first()).toHaveText(/🃏\s*1\/10/);
  });

  test('Replay restarts the current level with a fresh deck', async ({ page }) => {
    test.setTimeout(90000);
    await page.goto('/games/speed-match/play/2?seed=13');
    await solveDeck(page);
    await expect(page.locator('.win-overlay')).toBeVisible();
    ...
    await expect(page.locator('.win-overlay')).toBeHidden();
    await expect(page.locator('.hud-item').first()).toHaveText(/🃏\s*1\/10/);
    await expect(page).toHaveURL(/\/games\/speed-match\/play\/2/);
  });
